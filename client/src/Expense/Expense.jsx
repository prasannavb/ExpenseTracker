...
const ExpensesPage = () => {
  ...
  const [formData, setFormData] = useState({uid:`${sessionStorage.getItem('uid')}`,bid:`${sessionStorage.getItem('bid')}`, name: '', amount: '', date: '', type: '' });
  const [formErrors, setFormErrors] = useState({});
  const [budget_limit,setBudgetLimit]=useState({budget_limit:'',total_expenses:''})
  const openModal = (expense = null) => {
    setCurrentExpense(expense);
    setFormData(expense || { uid:`${sessionStorage.getItem('uid')}`,bid:`${sessionStorage.getItem('bid')}`, name: '', amount: '', date: '', type: '' });
    setFormErrors({});
    setIsModalOpen(true);
  };
  ...
  const closeModal = () => {
    ...
    setFormData({uid:`${sessionStorage.getItem('uid')}`,bid:`${sessionStorage.getItem('bid')}`, name: '', amount: '', date: '', type: '' });
    setFormErrors({});
  };
          ...
          <tbody>
            {expenses.map((expense) => (
              <tr key={expense.id}>
                ...
                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <p className="text-gray-900 whitespace-no-wrap">${parseFloat(expense.amount).toFixed(2)}</p>
                </td>
                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                ...
                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  ...
                  <button
                    onClick={() => deleteExpense(expense.expid,expense.bid)}
