...
const Dashboard = () => {
  ...
  const openModal = (mode, budget = null) => {
    ...
    setFormData(
      budget
        ? { uid: `${sessionStorage.getItem('uid')}`, budget_name: budget.budget_name, budget_limit: budget.budget_limit }
        : { uid: `${sessionStorage.getItem('uid')}`, budget_name: '', budget_limit: '' }
    );
    setFormErrors({});
    setIsModalOpen(true);
  };
  ...
  const validateForm = () => {
    ...
    let total_expense = Number(sessionStorage.getItem('total_expenses'));
    if (modalMode == 'edit' && total_expense > Number(formData.budget_limit)) {
      errors.budget_limit = 'Already spent ahead of budget';
    }
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex justify-end space-x-2">
                  <button onClick={() => { viewAllExpense(budget.bid, budget.budget_limit) }} className="text-green-600 hover:text-green-800 focus:outline-none">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="green" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" role="img">
                  ...
                  <button
                    onClick={() => {
                      ...
                      sessionStorage.setItem('total_expense', budget.total_expenses);
            ...
            <tbody>
              {expenses.map((expense) => (
                <tr key={expense.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{expense.type}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{expense.budget}</td>
                </tr>
              ))}
            </tbody>
          </table>
